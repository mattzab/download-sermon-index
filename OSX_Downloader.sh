...
curl -o cache.zip http://img.sermonindex.net/Cache_Update.zip
...
rm *.zip
echo  
echo Cleaning up XML files to be used with Curl...
echo  
...
echo 
echo download everything by using curl
for d in ./*/ ; do (cd "$d" && while read filename; do read url; curl -Lo $filename $url; done < *.xml); done
echo 
